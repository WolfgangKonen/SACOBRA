...
#updateCobraEqu
#'
#' Calculate \code{cobra$xbest,fbest,ibest} given the currently evaluated points.
#' 
#' First, we calculate the set S of points being feasible in the inequalities and feasible
#' with margin \code{cobra$currentEps} in the equalities. If S is non-empty, take the point from S 
#' with best objective as \code{cobra$xbest}. If S is empty (there are no feasible points): If the new
#' point has lower maxViol than the maxViol of \code{cobra$xbest}, take it as the new \code{cobra$xbest}. 
#' 
#' Called from \code{\link{updateSaveCobra}}.
#' 
#' @param cobra     an object of class COBRA (a long list, see \code{\link{cobraInit}})
#' @param xNew      the last evaluated point
#' 
#' @return \code{cobra}, an object of class COBRA, with potentially modified \code{cobra$xbest, 
