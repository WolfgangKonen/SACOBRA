...
isresCobyla <- function(xStart,fn=subProb2,hin=gCOBRA, cobra) {
  ...
  subMin2 <- nloptr::cobyla(xStart,fn=fn,lower=cobra$lower,upper=cobra$upper,hin=hin_c, control=list(maxeval=cobra$seqFeval,xtol_rel=cobra$seqTol), cobra=cobra,deprecatedBehavior=FALSE); 
