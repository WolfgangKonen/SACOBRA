...
# - cobra$PLOG is a boolean vector with length number of phase-II-iterations. The last element is set 
#   by adFit (SACOBRA.R) and adFit is called at the start of each iteration (see trainSurrogates.R)
...
cobraPhaseII <- function(cobra){
  ...
  cobra$MSXI<-c()
  cobra$WRITE_XI<-TRUE
  globalOptCounter = 0  # /WK/2025/03/06: changed from former setting 1 to be in sync with Python side and to start with the first element of DRC
  fn=cobra$fn
  ...
  adjustMargins <- function(Cfeas,Tfeas,Cinfeas,Tinfeas,EPS,epsMax,currentEps) {
    if(Cfeas >= Tfeas){
      EPS <<- EPS/2
      verboseprint(cobra$verbose, important=FALSE,sprintf("reducing epsilon to %f",EPS))   # EPS[1]
    ...
    if(Cinfeas>=Tinfeas){
      ...
      verboseprint(cobra$verbose, important=FALSE,sprintf("increasing epsilon to %f",EPS))   # EPS[1]
  ...
  while(num < cobra$feval){
    ...
    if(cobra$sac$RS){
      cobra<-cobra$rs_obj$random_start(cobra)
      xStart<-cobra$xStart
    ...
    switch(cobra$seqOptimizer,
         ...
         COBYLA={ subMin<-nloptr::cobyla(xStart,fn=subProb2,lower=cobra$lower,upper=cobra$upper,hin=cobra$gCOBRA_c,control=list(maxeval=cobra$seqFeval,xtol_rel=cobra$seqTol),cobra=cobra,deprecatedBehavior=FALSE); subMin$feval=subMin$iter },
         ISRESN={ subMin<-nloptr::isres(xStart,fn=subProb2,lower=cobra$lower,upper=cobra$upper,hin=cobra$gCOBRA_c, maxeval=cobra$seqFeval,xtol_rel=cobra$seqTol,cobra=cobra,deprecatedBehavior=FALSE); subMin$feval=subMin$iter },
    ...
    if (DBG_F02) {
      ...
      for (k in 0:min(3,d-1)) {
       ...
       # plot(xmat,fmat,type="l",main=sprintf("dimension=%s",k+1),ylim=c(0,max(c(fmat))),xlab=substitute(paste(x[i]),list(i=k+1)),ylab="f(x)")   # a plot along the x-axis, the narrow, flat 'valley'
        ...
        #plot(xmat*5,fmat,type="l",ylim=c(0,max(fmat)),xlab=substitute(paste(x[i]),list(i=k+1)),ylab=myylab,lwd = 5, cex=2,cex.axis=1.3,cex.lab=1.1)   # a plot along the x-axis, the narrow, flat 'valley' # cex.names=1.3,
        plot(xmat*5,fmat,type="l",xlab=substitute(paste(x[i]),list(i=k+1)),ylab=myylab,lwd = 5, cex=2,cex.axis=1.3,cex.lab=1.1)   # a plot along the x-axis, the narrow, flat 'valley'  # cex.names=1.3,
