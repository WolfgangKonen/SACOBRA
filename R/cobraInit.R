...
#'                      equality constraint handling described in \code{\link{defaultEquMu}()}. 
#'                      equHandle$active is set to TRUE by default.
...
#'      \item{\code{maxViol}}{ maximum constraint violation, see \code{\link{cobraPhaseII}}}
#'      \item{\code{trueMaxViol}}{ maximum constraint violation, see \code{\link{cobraPhaseII}}}
                      ...
                      cobraSeed=42 )
{
  ...
  sw=switch(initDesign,
         ...
         "OPTIMIZED" =  { # optimized (HJKB) initial points    
           ...
           # now the data for I, Fres, Gres are on initialHookeJeeves$history:
           ...
           colnames(Gres)<-names(fn(xStart))[-1]    # /WK/2025/03/27: bug fix
  ...
  cobra<-list(fn=fn,
              ...
              xStart=NULL,    # the start point for the next sequential optimization
              fName=fName,
              dimension=dimension, 
              ...
              feval=feval, 
              A=A, 
              #fbestArray=fbestArray,   # will be set below
              ...
              Fres=Fres, 
              Gres=Gres, 
              # numViol=numViol,        # will be set below
              # maxViol=maxViol,
              ...
              initDesFactory2=initDesFactory2,
              penaF=penaF,
              cobraSeed=cobraSeed,
              conTol=conTol,
 ...
 if(is.character(cobra$CA$ITER)) if(cobra$CA$ITER=="all")cobra$CA$ITER<-seq(initDesPoints,feval,1)
  ...
  numViol<-sapply(1:initDesPoints , function(i){ # number of initial constraint violations
    return(sum(cobra$Gres[i,] > cobra$conTol))
  })
  maxViol<-sapply(1:initDesPoints , function(i){
    #y<-max(0,cobra$Gres[i,])
    y<-max(cobra$conTol,cobra$Gres[i,])      # /WK/2025/03/23: bug fix
  ...
  if(length(equIndex)==0){
    ...
    maxViol<-sapply(1:initDesPoints , function(i){
      ...
      y = max(cobra$conTol, tempG[i,])        # /WK/2025/03/23: bug fix conTol
      return(y)
    })
    numViol<-sapply(1:initDesPoints , function(i){ # number of initial Constraint violations
      #return(sum(tempG[i,]-currentEps>0))
      return(sum(tempG[i,] > cobra$conTol))   # /WK/2025/03/23: bug fix conTol
    })
  } # else if(equHandle$active)
  

  cobra$numViol=numViol
  cobra$trueNumViol=numViol
